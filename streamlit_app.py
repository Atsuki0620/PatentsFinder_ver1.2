...
ユーザーが指定したIPCコードがなければ、chat_inputのキーワードから推測してIPCコードを提案します。
例："「逆浸透膜の機械学習についての事例」としては関連するIPCコードには、C02F 1/00（逆浸透膜の製造や利用）、G06N 3/04（機械学習に関連する知識処理）、G01N 33/569（逆浸透膜の性能評価）が考えられます。あなたが興味のあるのは、例えば「機械学習を用いた逆浸透膜の製造による高性能化」「逆浸透膜の性能評価を機械学習で高度予測する技術」などでしょうか。
